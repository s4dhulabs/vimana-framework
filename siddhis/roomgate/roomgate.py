...
from siddhis.roomgate.utils import should_show_banner, rg_banner
from siddhis.roomgate.engines.spec_manager import SpecResolutionError
from siddhis.roomgate.orchestrator import run_spec_scan, run_room_audit
...
class siddhi:
    ...
    def start(self):
        ...
        scan_only = handler.get('api_scan_enabled') and not handler.get('room_audit_enabled')
        ...
        if (
            handler.get('room_audit_enabled')
            or handler.get('target_url')
            or handler.get('room_path')
            or handler.get('ws_path')
        ):
            try:
                run_room_audit(handler)
                return True
            except SpecResolutionError as exc:
                self._fail(str(exc))
