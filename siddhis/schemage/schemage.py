...
from siddhis.schemage.utils import should_show_banner, sg_banner
from siddhis.schemage.engines.spec_manager import SpecResolutionError
from siddhis.schemage.orchestrator import run_spec_scan, run_gql_audit
...
class siddhi:
    ...
    def start(self):
        ...
        scan_only = handler.get('api_scan_enabled') and not handler.get('gql_audit_enabled')
        ...
        if (
            handler.get('gql_audit_enabled')
            or handler.get('target_url')
            or handler.get('gql_path')
        ):
            try:
                run_gql_audit(handler)
                return True
            except SpecResolutionError as exc:
                self._fail(str(exc))
