# -*- coding: utf-8 -*-
# OpenAPI resolution for socketline (delegates to core.vmnf_specs).
...
class SocketlineSpecManager:
    """Socketline-facing wrapper around canonical Vimana OpenAPI management."""

    ENV_PATH = os.path.expanduser('~/.socketline_env')
    PLUGIN = 'socketline'
    CACHE_SUBDIR = 'specs'
    ...
    def _save_env(self, spec_id: str, spec_info: dict) -> None:
        env_map = {
            'SOCKETLINE_SPEC_ID': spec_id,
            'SOCKETLINE_TARGET': spec_info['spec_host'],
            'SOCKETLINE_API_TITLE': spec_info['spec_title'],
            'SOCKETLINE_OPENAPI_VERSION': str(spec_info['openapi_version']),
            'SOCKETLINE_LAST_SCAN': spec_info['spec_date'].strftime('%Y-%m-%d %H:%M:%S'),
            'SOCKETLINE_SPEC_FILE': spec_info['spec_file_path'],
        }
    ...
    def _resolve_spec_id(self) -> Optional[str]:
        ...
        if self.handler.get('load_from_env') or any(
            ...
            return self._env.get('SOCKETLINE_SPEC_ID')
    ...
    def _resolve_target_url(self) -> Optional[str]:
        ...
        if scan == 'ENV_FALLBACK':
            return self._env.get('SOCKETLINE_TARGET')
    ...
    async def resolve(self) -> Tuple[dict, str, Optional[str]]:
        ...
        if scan == 'ENV_FALLBACK':
            if not target_url:
                raise SpecResolutionError('No target in ~/.socketline_env. Run --scan-api first.')
        ...
        raise SpecResolutionError(
            ...
            '--spec-file, or --ws-path with --target-url.'
        )
