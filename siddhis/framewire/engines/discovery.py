# -*- coding: utf-8 -*-
# WebSocket endpoint discovery for framewire (post-handshake targets).

import re
from typing import Dict, List, Set

from siddhis.framewire.utils import join_ws_url
...
def discover_from_openapi(api_specs: dict, base_url: str) -> List[Dict[str, str]]:
    ...
    for path, operations in api_specs.get('paths', {}).items():
        ...
        if 'websocket' in blob or path.startswith('/ws'):
            if path not in seen:
                seen.add(path)
                # Concrete room for template paths
                concrete = path.replace('{room_id}', 'framewire').replace('{id}', '1')
                found.append({
                    'path': concrete,
                    'source': 'openapi',
...
def resolve_targets(handler: dict, api_specs: dict, base_url: str) -> List[Dict[str, str]]:
    explicit = handler.get('frame_path') or handler.get('ws_path')
    if explicit:
