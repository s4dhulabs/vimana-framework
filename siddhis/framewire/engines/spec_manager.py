# -*- coding: utf-8 -*-
# OpenAPI resolution for framewire (delegates to core.vmnf_specs).
...
class FramewireSpecManager:
    """Framewire-facing wrapper around canonical Vimana OpenAPI management."""

    ENV_PATH = os.path.expanduser('~/.framewire_env')
    PLUGIN = 'framewire'
    CACHE_SUBDIR = 'specs'
    ...
    def _save_env(self, spec_id: str, spec_info: dict) -> None:
        env_map = {
            'FRAMEWIRE_SPEC_ID': spec_id,
            'FRAMEWIRE_TARGET': spec_info['spec_host'],
            'FRAMEWIRE_API_TITLE': spec_info['spec_title'],
            'FRAMEWIRE_OPENAPI_VERSION': str(spec_info['openapi_version']),
            'FRAMEWIRE_LAST_SCAN': spec_info['spec_date'].strftime('%Y-%m-%d %H:%M:%S'),
            'FRAMEWIRE_SPEC_FILE': spec_info['spec_file_path'],
        }
    ...
    def _resolve_spec_id(self) -> Optional[str]:
        ...
        if self.handler.get('load_from_env') or any(
            ...
            return self._env.get('FRAMEWIRE_SPEC_ID')
    ...
    def _resolve_target_url(self) -> Optional[str]:
        ...
        if scan == 'ENV_FALLBACK':
            return self._env.get('FRAMEWIRE_TARGET')
    ...
    async def resolve(self) -> Tuple[dict, str, Optional[str]]:
        ...
        if scan == 'ENV_FALLBACK':
            if not target_url:
                raise SpecResolutionError('No target in ~/.framewire_env. Run --scan-api first.')
