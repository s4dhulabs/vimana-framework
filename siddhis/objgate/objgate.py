...
from siddhis.objgate.utils import should_show_banner, og_banner
from siddhis.objgate.engines.spec_manager import SpecResolutionError
from siddhis.objgate.orchestrator import run_spec_scan, run_obj_audit
...
class siddhi:
    ...
    def start(self):
        ...
        scan_only = handler.get('api_scan_enabled') and not handler.get('obj_audit_enabled')
        ...
        if (
            handler.get('obj_audit_enabled')
            or handler.get('target_url')
            or handler.get('obj_path')
        ):
            try:
                run_obj_audit(handler)
                return True
            except SpecResolutionError as exc:
                self._fail(str(exc))
