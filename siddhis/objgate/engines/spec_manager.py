# -*- coding: utf-8 -*-
# OpenAPI resolution for objgate (delegates to core.vmnf_specs).
...
class ObjgateSpecManager:
    """Objgate-facing wrapper around canonical Vimana OpenAPI management."""

    ENV_PATH = os.path.expanduser('~/.objgate_env')
    PLUGIN = 'objgate'
    CACHE_SUBDIR = 'specs'
    ...
    def _save_env(self, spec_id: str, spec_info: dict) -> None:
        env_map = {
            'OBJGATE_SPEC_ID': spec_id,
            'OBJGATE_TARGET': spec_info['spec_host'],
            'OBJGATE_API_TITLE': spec_info['spec_title'],
            'OBJGATE_OPENAPI_VERSION': str(spec_info['openapi_version']),
            'OBJGATE_LAST_SCAN': spec_info['spec_date'].strftime('%Y-%m-%d %H:%M:%S'),
            'OBJGATE_SPEC_FILE': spec_info['spec_file_path'],
        }
    ...
    def _resolve_spec_id(self) -> Optional[str]:
        ...
        if self.handler.get('load_from_env') or any(
            ...
            return self._env.get('OBJGATE_SPEC_ID')
    ...
    def _resolve_target_url(self) -> Optional[str]:
        ...
        if scan == 'ENV_FALLBACK':
            return self._env.get('OBJGATE_TARGET')
    ...
    async def resolve(self) -> Tuple[dict, str, Optional[str]]:
        ...
        if scan == 'ENV_FALLBACK':
            if not target_url:
                raise SpecResolutionError('No target in ~/.objgate_env. Run --scan-api first.')
