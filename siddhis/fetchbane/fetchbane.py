...
from siddhis.fetchbane.utils import should_show_banner, fb_banner
from siddhis.fetchbane.engines.spec_manager import SpecResolutionError
from siddhis.fetchbane.orchestrator import run_spec_scan, run_ssrf_audit
...
class siddhi:
    ...
    def start(self):
        ...
        scan_only = handler.get('api_scan_enabled') and not handler.get('ssrf_audit_enabled')
        ...
        if (
            handler.get('ssrf_audit_enabled')
            or handler.get('target_url')
            or handler.get('ssrf_endpoint')
        ):
            try:
                run_ssrf_audit(handler)
                return True
            except SpecResolutionError as exc:
                self._fail(str(exc))
