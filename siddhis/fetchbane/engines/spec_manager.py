# -*- coding: utf-8 -*-
# OpenAPI resolution for fetchbane (delegates to core.vmnf_specs).
...
class FetchbaneSpecManager:
    """Fetchbane-facing wrapper around canonical Vimana OpenAPI management."""

    ENV_PATH = os.path.expanduser('~/.fetchbane_env')
    PLUGIN = 'fetchbane'
    CACHE_SUBDIR = 'specs'
    ...
    def _save_env(self, spec_id: str, spec_info: dict) -> None:
        env_map = {
            'FETCHBANE_SPEC_ID': spec_id,
            'FETCHBANE_TARGET': spec_info['spec_host'],
            'FETCHBANE_API_TITLE': spec_info['spec_title'],
            'FETCHBANE_OPENAPI_VERSION': str(spec_info['openapi_version']),
            'FETCHBANE_LAST_SCAN': spec_info['spec_date'].strftime('%Y-%m-%d %H:%M:%S'),
            'FETCHBANE_SPEC_FILE': spec_info['spec_file_path'],
        }
    ...
    def _resolve_spec_id(self) -> Optional[str]:
        ...
        if self.handler.get('load_from_env') or any(
            ...
            return self._env.get('FETCHBANE_SPEC_ID')
    ...
    def _resolve_target_url(self) -> Optional[str]:
        ...
        if scan == 'ENV_FALLBACK':
            return self._env.get('FETCHBANE_TARGET')
    ...
    async def resolve(self) -> Tuple[dict, str, Optional[str]]:
        ...
        if scan == 'ENV_FALLBACK':
            if not target_url:
                raise SpecResolutionError('No target in ~/.fetchbane_env. Run --scan-api first.')
