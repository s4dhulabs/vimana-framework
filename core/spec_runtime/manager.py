# -*- coding: utf-8 -*-
# Plugin-agnostic OpenAPI SpecManager (replaces 8 near-identical plugin copies).
...
from core.spec_runtime.quiet import is_quiet


class SpecResolutionError(Exception):
    """Raised when OpenAPI/spec context cannot be resolved."""


class SpecManager:
    """
    Canonical OpenAPI resolution / registration wrapper.

    Plugins provide only ``plugin`` name (and optional env_path / cache_subdir).
    """
    ...
    def __init__(
        ...
        *,
        ...
        cache_subdir: str = 'specs',
        env_prefix: Optional[str] = None,
    ):
        self.handler = handler or {}
        self.plugin = plugin
        self.CACHE_SUBDIR = cache_subdir
        self.ENV_PATH = env_path or os.path.expanduser(f'~/.{plugin}_env')
        self.env_prefix = (env_prefix or plugin).upper()
        self.quiet = is_quiet(self.handler)
        self._env = self._load_env()

    def _env_key(self, suffix: str) -> str:
    ...
    def _save_env(self, spec_id: str, spec_info: dict) -> None:
        env_map = {
            self._env_key('SPEC_ID'): spec_id,
            self._env_key('TARGET'): spec_info['spec_host'],
            self._env_key('API_TITLE'): spec_info['spec_title'],
            self._env_key('OPENAPI_VERSION'): str(spec_info['openapi_version']),
            self._env_key('LAST_SCAN'): spec_info['spec_date'].strftime('%Y-%m-%d %H:%M:%S'),
            self._env_key('SPEC_FILE'): spec_info['spec_file_path'],
        }
    ...
    def _resolve_spec_id(self) -> Optional[str]:
        ...
        if self.handler.get('load_from_env') or any(
            ...
            return self._env.get(self._env_key('SPEC_ID'))
    ...
    def _resolve_target_url(self) -> Optional[str]:
        ...
        if scan == 'ENV_FALLBACK':
            return self._env.get(self._env_key('TARGET'))
    ...
    async def resolve(self) -> Tuple[dict, str, Optional[str]]:
        ...
        if scan == 'ENV_FALLBACK':
            if not target_url:
                raise SpecResolutionError(
                    f'No target in {self.ENV_PATH}. Run --scan-api first.'
                )
        ...
        raise SpecResolutionError(
            ...
            '--spec-file, or a specialty path with --target-url.'
        )
